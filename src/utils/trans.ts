 ...
 * @LastEditTime: 2021-04-09 19:39:29
 * @LastEditors:
 */
import Vue from 'vue'
...
function trans(
  ...
  category?: boolean,
  source?: string,
  porp?: { codeKey: string; valueKey: string }
) {
  ...
  if (storeData.length > 0) {
    if (source && source.indexOf('/') > -1) {
      ...
      if (data && data.data) {
        ...
        if (category) {
          ...
          if (data.data.length > 0) {
            return data.data.filter(item => item[dataProp.codeKey] === key)[0]
              ? data.data.filter(item => item[dataProp.codeKey] === key)[0][dataProp.valueKey]
              : 'Error config!'
          } else {
            return '获取中...'
          }
      ...
      if (category) {
        ...
        if (store.length > 0) {
          if (
            store.filter(item => item.code === key)[0] &&
            store.filter(item => item.code === key)[0].id
          ) {
            return store.filter(
              dic => dic.pid === store.filter(item => item.code === key)[0].id.toString()
            )
          } else {
