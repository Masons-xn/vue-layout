<template>
  <div :id="unitid">
    ...
    >
      <el-select v-model="value" placeholder="请选择" v-if="draggable" value="">
        <el-option
          v-for="item in option"
          :key="item.value"
          :label="formtter(item, 'optionlabel')"
          :value="formtter(item, 'optionvalue')"
        >
        </el-option>
      </el-select>
      <el-select
...
export default class RbSelect extends Vue {
  ...
  @Prop({ default: 'id', dataType: 'string', valueBus: [], des: '存储key', alias: '数据键' } as any)
  private dataKey: string
  @Prop({ default: '', dataType: 'string', valueBus: [], des: '存储值', alias: '存储值' } as any)
