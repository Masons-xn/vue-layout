    ...
    <el-dialog :title="tips"
      ...
      <div>
        <model-add :id="editId"
                   ref="add"
                   :models="model"
                   v-on:pageLoad="pageLoad"></model-add>
      </div>
    </el-dialog>
  </g-page>
</template>
...
export default class Dictionary extends Vue {
  ...
  public loadTree() {
    this.model.map((model: { type: string; label: string; stName: string; stTableName: string }) => {
      model.type = "model"
      model.label = `${model.stName}-${model.stTableName}`
    })
    this.modelBase.map((base: { stModelId: string; type: string; label: string; stName: string; stColumnName: string }) => {
      const model: any = this.model.filter((mo: { id: string; label: string; stName: string; stTableName: string }) => mo.id === base.stModelId)[0] || { children: [] }
      base.type = "base"
      base.label = `${base.stColumnName}(${base.stName})`
      if (model) {
        model.children && model.children.length ? model.children.push(base) : (model.children = [base])
      }
    })
    this.modelRel.map((rel: any) => {
      rel.type = "rel"
      const model: { children: any[] } = this.model.filter((mo: { id: string; label: string; stName: string; stTableName: string }) => mo.id === rel.stSouModelID)[0] || { children: [] }
      rel.alias = rel.stName
      rel.label = (rel.inRelation === 1 ? "(n)" : "(1)") + (this.model.filter((mo: any) => mo.id === rel.stDestModelID)[0] as any).stName
      if (model) {
        model.children && model.children.length ? model.children.push(rel) : (model.children = [rel])
      }
    })
  ...
  public getRel(nodes: any[], links: any[]) {
    ...
    this.modelRel.map((rel: { id: string; stSouModelID: string; stDestModelID: string; inRelation: number }) => {
      links.push({
        id: rel.id,
      ...
      <div class="custom-tree-node geye-100" flex>
        ...
        <div flex-box="0" flex="main:right" v-show={node.level === 1}>
          <el-tooltip content="编辑" placement="top" effect="light" openDelay={300}>
            <g-icon name="md-color-filter" size="18" on-press={(e: { stopPropagation: () => void }) => this.handleEdit(e, data)} />
          </el-tooltip>
          <el-tooltip content="删除" placement="top" effect="light" openDelay={300}>
            <g-icon name="Rectangle" size="18" on-press={(e: { stopPropagation: () => void }) => this.handleRemove(e, data)} />
          </el-tooltip>
        </div>
      </div>
    )
